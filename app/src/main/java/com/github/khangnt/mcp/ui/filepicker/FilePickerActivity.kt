...
class FilePickerActivity : SingleFragmentActivity(), FileBrowserFragment.Callbacks {
    ...
    private val ensureReadable by lazy {
        intent.getBooleanExtra(EXTRA_ENSURE_FOLDER_READABLE, true)
    }
    private val ensureWritable by lazy {
        intent.getBooleanExtra(EXTRA_ENSURE_FOLDER_READABLE, false)
    }
    ...
    override fun onFragmentCreated(fragment: Fragment, savedInstanceState: Bundle?) {
        ...
        pathIndicatorView.onPathClick = { _, directory -> fileBrowserFragment.goto(directory) }
        ...
        btnSelect.setOnClickListener {
            ...
            if (isPickFile) {
                ...
                result.putExtra(
                    DIRECTORY_RESULT,
                    fileBrowserFragment.getCurrentDirectory().absolutePath,
                )
            }
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.item_goto_internal_storage -> {
                (getContentFragment() as? FileBrowserFragment)?.goto(
                    Environment.getExternalStorageDirectory()
                )
                return true
            }
