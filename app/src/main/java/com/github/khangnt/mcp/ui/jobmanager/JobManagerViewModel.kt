...
class JobManagerViewModel(appContext: Context) : ViewModel() {
    private val runningHeaderModel =
        LiveHeaderModel(
            appContext.getString(R.string.header_running_job),
            RunningJobStatus.observeRunningJobStatus(),
        )
    private val preparingHeaderModel =
        HeaderModel(appContext.getString(R.string.header_preparing_job))
    ...
    private val finishedHeaderModel =
        HeaderModel(appContext.getString(R.string.header_finished_job))
    ...
    fun reload() {
        ...
        with(SingletonInstances.getJobRepository()) {
            disposable =
                Flowable.combineLatest(getIncompleteJobs(), getCompletedJobs(), combineFunction())
    ...
    private fun combineFunction() =
        BiFunction<List<Job>, List<Job>, List<AdapterModel>> { list1, list2 ->
