...
class DefaultJobRepository(private val jobDao: JobDao) : JobRepository {
    private val singleThreadScheduler: Scheduler =
        Schedulers.from(newSingleThreadExecutor { Thread(it, "JobDatabaseThread") })

    private val completedJobListChangeEvent = BehaviorSubject.createDefault(Unit)

    override fun addJob(job: Job): Single<Job> =
        Single.fromCallable { jobDao.insertJob(job) }
            .map { job.copy(id = it) }
    ...
    override fun nextReadyJob(): Single<Optional<Job>> =
        jobDao
            .getReadyJob()
            .map { it.asOptional() }
    ...
    override fun nextPendingJob(): Single<Optional<Job>> =
        jobDao
            .getPendingJob()
            .map { it.asOptional() }
            .onErrorReturn { Optional.absent() }
    ...
    override fun getCompletedJobs(): Flowable<List<Job>> =
        completedJobListChangeEvent
            .toFlowable(BackpressureStrategy.LATEST)
            .map { jobDao.getCompletedJobs() }
            .subscribeOn(singleThreadScheduler)

    override fun getIncompleteJobs(): Flowable<List<Job>> =
        jobDao.getIncompleteJobs().subscribeOn(singleThreadScheduler)

    override fun getJobsByStatus(vararg status: Int): Single<List<Job>> =
        jobDao.getJobByStatus(*status).subscribeOn(singleThreadScheduler)
