...
/** Created by Khang NT on 1/22/18. Email: khang.neon.1997@gmail.com */
data class WorkingPaths(
    ...
    fun getTempDirForJob(jobId: Long): File =
        File(jobTempRootDir, jobId.toString()).ensureDirExists()
...
/**
 * Working paths can change any times (app move to other storage, sdcard mount/unmounted,...) Jobs
 * which affected when working paths changed should be fail with message: "Some app data deleted or
 * moved."
 */
...
fun makeInputTempFile(jobTempDir: File, inputIndex: Int): File =
    File(jobTempDir, "input$inputIndex")
