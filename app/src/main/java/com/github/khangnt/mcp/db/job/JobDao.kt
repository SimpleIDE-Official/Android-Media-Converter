...
interface JobDao {
    @Query(
        """SELECT * FROM jobs_v2 WHERE status == $COMPLETED OR status == $FAILED
        ORDER BY _id DESC"""
    )
    fun getCompletedJobs(): List<Job>

    @Query(
        """SELECT * FROM jobs_v2 WHERE status != $COMPLETED AND status != $FAILED
        ORDER BY status DESC, _id ASC"""
    )
    fun getIncompleteJobs(): Flowable<List<Job>>

    @Insert(onConflict = OnConflictStrategy.FAIL) fun insertJob(job: Job): Long

    @Update fun updateJob(job: Job)

    @Query("DELETE FROM jobs_v2 WHERE _id == :jobId") fun deleteJob(jobId: Long)
