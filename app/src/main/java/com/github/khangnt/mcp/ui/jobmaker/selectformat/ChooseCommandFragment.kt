...
/** Created by Khang NT on 4/7/18. Email: khang.neon.1997@gmail.com */
class ChooseCommandFragment : StepFragment() {
    ...
    private val adapter: MixAdapter by lazy {
        MixAdapter.Builder {
                ...
                withModel<EditCommandModel> {
                    ItemEditCommandViewHolder.Factory { onClick = { onSelectEditCommand(it) } }
                }
            }
            .build()
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        jobMakerViewModel.getSelectedFiles().observe { adapter.setData(makeAdapterData(it.size)) }
    }
    ...
    private fun makeAdapterData(selectedFileCount: Int): List<AdapterModel> {
        ...
        adapterModels.addAll(
            ConvertCommand.values().map { ConvertCommandModel(it, selectedFileCount > 0) }
        )
