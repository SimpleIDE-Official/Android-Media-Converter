...
fun copy(input: InputStream, output: OutputStream, bufferLength: Int = DEFAULT_IO_BUFFER_LENGTH) {
...
fun sendEmail(context: Context, subject: String = "", msg: String = "") {
    val emailIntent =
        Intent(Intent.ACTION_SENDTO, Uri.fromParts("mailto", "help.mediaconverter@gmail.com", null))
...
fun String.parseInputUri(): Uri {
    if (startsWith("http", ignoreCase = true) || startsWith("content", ignoreCase = true)) {
...
/**
 * Check if [filename] exists in [folderUri]. return Uri of the file existing, otherwise return null
 */
...
fun viewChangelog(context: Context) {
    ...
    (webView.layoutParams as ViewGroup.MarginLayoutParams).setMargins(margin, 0, margin, 0)
